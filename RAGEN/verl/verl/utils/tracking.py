...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            wandb.init(project=project_name, name=experiment_name, config=config, mode=mode, dir=wandb_dir)
            self.logger["wandb"] = wandb
            if mode == "offline" and TriggerWandbSyncHook is not None:
                comm_dir = os.environ.get(
                    "WANDB_OSH_COMM_DIR",
                    os.path.expanduser("~/.wandb_osh_communication")
                )
                self._wandb_trigger = TriggerWandbSyncHook(communication_dir=comm_dir)
    ...
    def maybe_sync(self):
        import os
        wb = self.logger.get("wandb")
        if not wb or not getattr(self, "_wandb_trigger", None):
            return
        logdir = getattr(wb.run, "dir", None)
        run_root = os.path.abspath(os.path.join(logdir, "..")) if logdir else None
        self._wandb_trigger(logdir=run_root)
