...
class LLMAgentProxy:
	...
	def generate_sequences(self, lm_inputs: DataProto):
		...
		print("llm inputs: ", lm_inputs)
		print("llm outputs: ", lm_outputs)
		return lm_outputs
	...
	def rollout(self, dataproto: DataProto, val=False):
		...
		meta = dataproto.meta_info or {}
		env_outputs = es_manager.reset(meta_info=meta)
		...
		accel_infos = es_manager.collect_accel_infos()
		print("accel_infos: ", accel_infos) # for testing
		rollouts = ctx_manager.formulate_rollouts(rollout_states)
		rollouts.meta_info = (rollouts.meta_info or {})
		rollouts.meta_info["accel_env_info"] = accel_infos
